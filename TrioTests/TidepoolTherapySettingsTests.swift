...
@Suite("Tidepool Glucose Upload Coordination") struct TidepoolGlucoseUploadCoordinatorTests {
    @Test("Overlapping requests become one follow-up upload") func overlappingRequestsAreCoalesced() async {
        let coordinator = TidepoolGlucoseUploadCoordinator()
        let probe = TidepoolUploadProbe()
        ...
        await probe.waitUntilFirstUploadIsBlocked()

        // These requests arrive while the first upload is waiting for its
        // network callback. They should request one follow-up pass, not five
        // concurrent uploads of the same glucose.
        for _ in 0 ..< 5 {
            await coordinator.perform {
                await probe.run()
            }
        ...
        await probe.releaseFirstUpload()
        await firstUpload.value

        #expect(await probe.invocationCount == 2)
        #expect(await probe.maximumConcurrentInvocations == 1)
    }
}

private actor TidepoolUploadProbe {
    private(set) var invocationCount = 0
    private(set) var maximumConcurrentInvocations = 0
    private var activeInvocations = 0
    private var firstUploadIsBlocked = false
    ...
    func waitUntilFirstUploadIsBlocked() async {
        while !firstUploadIsBlocked {
            await Task.yield()
        }
    }

    func releaseFirstUpload() {
        firstUploadContinuation?.resume()
